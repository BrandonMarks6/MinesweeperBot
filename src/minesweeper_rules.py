...
class MinesweeperRules:
    ...
    def check_around_to_clear(self, board, row, col, max_row, max_col):
        value = (board[row][col]).val

        if value not in ('.', 'X', '-'):
            # gets number of green spaces and flag spaces
            holder = self.get_green_and_flag(board, row, col, max_row, max_col)
            flag_count = holder[1]
    ...
    def check_around_to_mark(self, board, row, col, max_row, max_col):
        ...
        if value not in ('.', 'X', '-'):
            ...
            if (green_count + flag_count == int(value)) and int(value) - flag_count != 0:
                print("Add a flag to every spot around " +
                    str(col) + ", " + str(row))
                self.action_object.right_click_all_around(board, row, col, max_row, max_col)
                return True
    ...
    def get_green_and_flag(self, board, row, col, max_row, max_col):
        flag_counter = self.check_for_char("X", board, row, col, max_row, max_col)
        green_counter = self.check_for_char("-", board, row, col, max_row, max_col)
    ...
    def check_for_char(self, curr_char, board, row, col, max_row, max_col):
        counter = 0
